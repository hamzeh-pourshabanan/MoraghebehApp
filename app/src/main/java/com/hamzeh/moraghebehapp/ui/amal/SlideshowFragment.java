...
//    public PrimeDatePicker datePicker;  // installed in FragmentComponent
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        binding.currentDayTv.setText(FaNum.convert(String.valueOf(resultsCount + 1)));
...
//            String current = String.valueOf(dayNumber).concat("(").concat(mCurrentDate.getShortDateString()).concat(")");
    ...
    private void updateUI(int postId){
        if (postId != 0 ) {
            Toast.makeText(getContext(), "اعمال امروز با موفقیت ثبت شد." , Toast.LENGTH_LONG).show();
            resultsCount++;
            updateUIWhenStopingHTTPRequest(resultsCount+1);
        } else {
            Toast.makeText(getContext(), "اعمال امروز ثبت نشد!", Toast.LENGTH_LONG).show();
        }
